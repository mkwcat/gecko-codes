...
enum class ProfileName {
    ...
    AC_LIGHT_WATER = 618,
    AC_COPYRIGHT = 619,
    // Actor 620, in the Nvidia Shield version, is replaced with three new
    // actors. These are unimplemented at the moment, but are worth noting.

    // LOGO_SCREEN = 620,
    // NV_SCREEN = 621,
    // ANTI_ADDICTION_SCREEN = 622,

    // Yes, this does shift the entire table by two (WM_CS_SEQ_MNG becomes 623
    // and so on). translateActorID() will need to be modified to support this,
    // and more vigorously applied if/when support for the Shield version is
    // implemented.
    WII_STRAP = 620,
    WM_CS_SEQ_MNG = 621,
    WORLD_CAMERA = 622,
    ...
    // TIME_UP is 702 in earlier versions and that must be accounted for when
    // using this
