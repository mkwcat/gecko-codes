...
extern u8 oslow_30e3 asm("0x800030E3");
extern u16 oslow_30e0 asm("0x800030E0");
...
// PAL: 0x803869a0
u32 PAD_Spec;
extern u32 __PADFixBits asm("PORT(0x8042AA8C)");
...
{
    u32 chanBit;
    PADResetChan = __builtin_clz(PADResetBits);
    if (PADResetChan == 32)
        return;
...
#pragma unused(chan)
    ...
    if (!(type & 0x80000000u) || (type & 0x04000000u)) {
        if (variant1) {
            ok = SITransfer(
                PADResetChan, &PAD_Unk80385b24, 3, &Origin[PADResetChan], 10,
                PADOriginCallback, 0
            );
        } else {
            ok = SITransfer(
                PADResetChan, &PAD_Unk80385b20, 1, &Origin[PADResetChan], 10,
                PADOriginCallback, 0
            );
        }
    } else if ((type & 0x00100000u) && (type & 0x00080000u) == 0x00000000u && !(type & 0x00040000u)) {
        if (type & 0x40000000u) {
            ok = SITransfer(
                PADResetChan, &PAD_Unk80385b20, 1, &Origin[PADResetChan], 10,
                PADOriginCallback, 0
            ...
            ok = SITransfer(
                PADResetChan, &PAD_Unk803481f0[PADResetChan], 3,
...
static void PADReceiveCheckCallback(s32 chan, u32 type)
{
    ...
    if (!(error & 0x000f) && (type & 0x80000000u) && (type & 0x00100000u) &&
        ...
        SITransfer(
            chan, &PAD_Unk80385b20, 1, &Origin[chan], 10,
...
#define abs __builtin_abs
        if ((abs(abs(status->stickX) - abs(PAD_AltStatus[i].stickX))) >=
                thres ||
            (abs(abs(status->stickY) - abs(PAD_AltStatus[i].stickY))) >=
                thres ||
            ...
            // __VIResetSIIdle();
            extern s32 __VIDimmingFlag_SI_IDLE asm("PORT(0x8042AB30)");
...
#undef abs
        ...
        if (status->button & 0x2000) {
            ...
            memset(status, 0, 0xa);
            SITransfer(
                i, &PAD_Unk80385b20, 1, &Origin[i], 10, PADOriginUpdateCallback,
                0
            );
            continue;
        }
